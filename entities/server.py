...
class Server:
    ...
    def train_round(self, clients):
        ...
        for i, c in enumerate(clients):
            ...
            updates.append((num_samples, copy.deepcopy(client_update), client_loss, c,
                            sparsity))  # deep copy to not change the original dictionary of client
        return updates
    ...
    def train(self):
        ...
        if self.args.tuning == True:
            ...
            if self.args.loo:
                train_csv.to_csv(
                    f'Federated_Non-IID:{self.args.niid}_LocalEpochs:{self.args.num_epochs}_Lr:{self.args.lr}_momentum:{self.args.m}_wd:{self.args.wd}_batchSize:{self.args.bs}_rounds:{self.args.num_rounds}_angle:{self.angle}_numClients:{self.args.clients_per_round}.csv',
                    index=False)
            else:
                train_csv.to_csv(
                    f'Federated_Non-IID:{self.args.niid}_LocalEpochs:{self.args.num_epochs}_Lr:{self.args.lr}_momentum:{self.args.m}_wd:{self.args.wd}_batchSize:{self.args.bs}_rounds:{self.args.num_rounds}_numClients:{self.args.clients_per_round}.csv',
                    index=False)
        else:
            train_dict = {'Epochs': np.array(range(self.args.num_rounds)), 'Train accuracy': np.array(train_accuracyp),
                          'Test accuracy': np.array(test_accuracyp)}
